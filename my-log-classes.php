...
class My_Log_Entry_Template {
	...
	public static function display_subject( $item, $col_type )
	{
		if ( $col_type == 'subject' )
		{
			...
			$subject .= '<div id="msgContentPopup'.$item->id.'" style="display:none"><h2>'.$item->action.'</h2><div style="width:100%;clear:both;display:inline-block;">'.nl2br($item->content).'</div>';
		}
		return $subject;
	}
	...
	public static function display_user( $item, $col_type )
	{
		...
		if($user_fullname){
		$user_item = '<a href="' . $link . '">' . $img . '</a><a href="' . $link . '">' . $user_fullname. '</a>';
		}else{
			$user_item = '<a href="' . $link . '">' . $img . '</a><a href="' . $link . '">'. $user_email. '</a>';
		}
