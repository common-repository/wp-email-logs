	...
	if($debug_trace)
	{
		foreach($debug_trace as $key=>$val)
		{
			...
			if($val['function']=='wp_mail')
			{
				...
				$emails_arr = array();
				if($from_email || $args['to']){
					$user_email_ids = array();
					if($from_email){
						$user_email_ids[] = '"'.$from_email.'","'.$args['to'].'"';
					}
					if($args['to']){
						$user_email_ids[] = '"'.$args['to'].'"';
					}
					if($user_email_ids){
						$user_email_ids_str = implode(',',$user_email_ids);
						$user_email_ids = '"'.$from_email.'","'.$args['to'].'"';
						$res = $wpdb->get_results("select ID,user_email from $wpdb->users where user_email in ($user_email_ids_str)");
